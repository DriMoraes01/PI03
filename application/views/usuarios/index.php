                    ...
                    <div class="card">
                        <div class="card-header d-block">
                            <a class="btn float-right text-white" style=" background-color: DarkOrange;"
                                ...
                                href="<?= base_url($this->router->fetch_class() . '/cadastrar/'); ?>"
                                aria-label="Cadastrar novo <?= $this->router->fetch_class(); ?>">
                                + Novo
                            </a>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive-sm">
                                <table class="table data-table table-sm pl-20 pr-20" aria-describedby="userTableDescription">
                                    <caption id="userTableDescription">Tabela de usuários cadastrados, com detalhes como e-mail, nome, perfil de acesso e status de atividade.</caption>
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        <?php foreach ($usuarios as $user) : ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?= ($user->active == 1
                                                        ? '<span class="badge badge-pill badge-success mb-1" aria-label="Ativo"><i class="fas fa-lock-open" aria-hidden="true"></i>&nbsp;Sim</span>'
                                                        : '<span class="badge badge-pill badge-warning mb-1" aria-label="Inativo"><i class="fas fa-lock" aria-hidden="true"></i>&nbsp;Não</span>'); ?>
                                                </td>
                                                <td class="text-right">
                                                    <?php if ($this->ion_auth->is_admin()) : ?>
                                                        <a data-toggle="tooltip"
                                                            data-placement="bottom"
                                                            title="Editar <?= $this->router->fetch_class(); ?>"
                                                            href="<?= base_url($this->router->fetch_class()) . '/editar/' . $user->id; ?>"
                                                        ...
                                                        <a data-toggle="tooltip"
                                                            data-placement="bottom"
                                                            title="Visualizar <?= $this->router->fetch_class(); ?>"
                                                            href="<?= base_url($this->router->fetch_class()) . '/visualizar/' . $user->id; ?>"
                                                            class="btn btn-icon btn-primary"
                                                            aria-label="Visualizar <?= $user->first_name; ?>">
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            aria-label="Excluir <?= $user->first_name; ?>">
                                                            <i class="ik ik-trash-2" aria-hidden="true"></i>
                                                        </button>
                                                    <?php endif; ?>
                                                </td>
                                            </tr>
                                            <div class="modal fade" id="user-<?= $user->id; ?>" tabindex="-1" role="dialog" aria-labelledby="modalLabel-<?= $user->id; ?>" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="modalLabel-<?= $user->id; ?>"><i class="fas fa-exclamation-triangle text-danger" aria-hidden="true"></i>&nbsp;Tem certeza que quer excluir?</h5>
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <a data-toggle="tooltip"
                                                                data-placement="bottom"
                                                                title="Excluir <?= $this->router->fetch_class(); ?>"
                                                                href="<?= base_url($this->router->fetch_class()) . '/del/' . $user->id; ?>"
                                                                class="btn btn-danger"
                                                                aria-label="Confirmar exclusão de <?= $user->first_name; ?>">
                                                                Sim, excluir
                                                            </a>
                                                        </div>
                                                    </div>
